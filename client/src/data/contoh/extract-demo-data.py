...
# Susunan 22 kolom format 2026, docs/rujukan/migrasi-data.md bagian 4.1.
KOLOM = {
    "B": "nik",
    "C": "nama",
    "D": "anak_ke",
...
# --------------------------------------------------------------------------
# Normalisasi — docs/rujukan/migrasi-data.md bagian 5
# --------------------------------------------------------------------------
...
def tanggal(nilai: str | None) -> date | None:
    ...
    meskipun docs/rujukan/migrasi-data.md bagian 5.1 menduga hanya arsip 2025 yang
    berbentuk serial.
    """
    if nilai is None:
        return None
...
def kategori(indeks: str, z: float) -> str | None:
    """Ambang PMK No. 2 Tahun 2020, sesuai docs/rujukan/ui-ux.md bagian 3.
...
def kunci_identitas(baris: dict) -> tuple:
    ...
    docs/rujukan/migrasi-data.md bagian 6. Tanpa penggabungan otomatis.
    """
    if baris["nik"]:
...
def kehadiran(baris: dict) -> tuple[str, dict[str, str]]:
    ...
    docs/rujukan/migrasi-data.md bagian 5.4. Tanda hubung dipakai sebagai penanda
...
def kelompokkan(mentah: list[dict]) -> dict[tuple, list[dict]]:
    ...
    Pencocokan dasarnya mengikuti docs/rujukan/migrasi-data.md bagian 6, tetapi ada
        ...
        anak.append(
            {
                ...
                # dikonversi diam-diam (docs/rujukan/migrasi-data.md bagian 5.5).
...
# metode pencarian tabel yang berbeda, lihat docs/riwayat/catatan-tahap-demo.md.
KOLOM_MASTER = {
    "BB_U": ("JUN_BB_U", "P"),
    "TB_U": ("JUN_TB_U", "Q"),
    "LIKA_U": ("JUN_LIKA_U", "T"),
