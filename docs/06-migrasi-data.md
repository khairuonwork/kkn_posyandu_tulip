...
Spesifikasi perintah `php artisan posyandu:import`. Perintah ini memindahkan arsip 2025–2026 ke basis data Portal.
...
```bash
php artisan posyandu:import <path> [--periode=YYYY-MM] [--dry-run] [--batch=]
```
...
Setiap aturan di bawah punya *test* tersendiri di `tests/Feature/Import/ImportArsipTest.php`.
...
|---|---|---|
| Berkas 2026 | String ISO `2026-01-20` | `Carbon::parse` dengan format eksplisit. |
...
Kolom `UMUR LENGKAP` dan `UMUR HARI` diabaikan sepenuhnya. Umur dihitung dari `tgl_lahir` dan `tanggal_ukur` (DR-03, [04-spesifikasi-antropometri.md](04-spesifikasi-antropometri.md) bagian 3.1).
...
Membaca `.xlsx` menuntut satu paket baru: **`openspout/openspout`**.

Alasan memilihnya:

- Tidak punya dependensi lain, dan jauh lebih ringan dari PhpSpreadsheet.
- Membaca secara *streaming*, sehingga berkas 1.054 baris tidak dimuat seluruhnya ke memori.
- Menangani *shared strings* dan tanggal Excel dengan benar.

Alasan **tidak** menulis pengurai sendiri di atas `ZipArchive` + `SimpleXML`: meski berkasnya sederhana, kesalahan urai yang senyap pada data kesehatan anak akan merusak rekam medis tanpa gejala. Ini termasuk validasi di batas kepercayaan, dan itu bukan tempat untuk berhemat kode.

*Export* tidak menambah dependensi: CSV ditulis dengan `fputcsv` bawaan PHP, diawali BOM UTF-8 agar Excel membacanya dengan benar.
