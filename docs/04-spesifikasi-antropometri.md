...
Dokumen ini adalah spesifikasi teknis mesin perhitungan status gizi. Isinya mengikat implementasi di `app/Support/Antropometri/` dan diverifikasi oleh `tests/Feature/Antropometri/ZScoreTest.php`.
...
Sampai pemilik program mengonfirmasi, sistem **menghitung dan menampilkan z-score LILA/U tanpa memberi label kategori**. Lihat [99-open-issues.md](99-open-issues.md) isu OI-04.
...
Nilainya diekstrak sekali ke `database/data/who-lms.json`, yang **di-*commit* ke repo** dan menjadi sumber *seed* tabel `standar_lms`.
...
Lokasi: `app/Support/Antropometri/`.
...
*Acceptance test* utama seluruh sistem: `tests/Feature/Antropometri/ZScoreTest.php`.
...
Cara mengulang verifikasi: seluruh nilai harapan tercatat sebagai *dataset* di `tests/Feature/Antropometri/ZScoreTest.php`, dalam bentuk anonim (jenis kelamin, kunci tabel, nilai ukur, z harapan) tanpa nama maupun NIK.
