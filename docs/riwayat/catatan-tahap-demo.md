...
lebih luas tetap tinggal di [`pertanyaan-terbuka.md`](../pertanyaan-terbuka.md); di sini hanya
...
- **TB/U Tinggi.** Master memberi label `Tinggi` pada z +1,16 sampai +1,76,
  ...
  temuan [OI-11](../pertanyaan-terbuka.md), dan [ADR-0002](../adr/0002-metode-z-score-who-lms.md)
...
| | |
|---|---|
| **Pemilik** | pemilik berkas standar / Bidan |
...
| Cara | Dipakai oleh |
|---|---|
| Interpolasi linear antara dua baris | mesin gizi (kini `server/src/antropometri/tabel-standar.ts`) dan skrip ekstraksi ini |
...
tertulis di `server/src/antropometri/tabel-standar.ts` dan [`rujukan/antropometri.md`](../rujukan/antropometri.md).
Mengubahnya hanya agar cocok dengan Excel berarti menyalin cara baca yang belum
tentu disengaja.

**Bukti tambahan, 21 September 2026.** Ditemukan saat port mesin gizi ke TypeScript ([ADR-0006](../adr/0006-pindah-ke-express-react-postgres.md)). Baris dataset pada `tests/Feature/Antropometri/ZScoreTest.php` menuntut `z ≈ 0,1993` untuk anak perempuan setinggi **61,4 cm** seberat 6,24 kg. Angka 0,1993 ternyata persis nilai pada baris **61,0** — jadi pembulatan ke bawah bukan hanya dipakai master Excel, tetapi juga sudah ikut tersalin ke dalam test aplikasi tanpa disadari. Interpolasi memberi 0,02408; selisihnya 0,175, jauh lebih besar daripada toleransi 0,01 yang dipakai test itu.

**Yang dibutuhkan:** pertanyaannya kini hidup sebagai **pertanyaan nomor 2** di [`rencana-kerja.md`](../rencana-kerja.md) dan tidak diulang di sini. Entri D-02 ini tinggal sebagai **buktinya** — angka 29-dari-87 dan temuan 61,4 cm di atas tidak tercatat di tempat lain.
...
> **Diredaksi 22 September 2026.** Tabel di bawah semula memuat NIK dan nama empat anak **sungguhan** dari arsip. NIK adalah data pribadi menurut dokumen proyek ini sendiri ([OI-10](../pertanyaan-terbuka.md)), dan repo ini bisa menjadi publik. Nomor dan namanya tidak disalin ke sini; keduanya ada di arsip sumber, di luar repo. Bentuk temuannya utuh — itulah yang perlu dibaca.

| NIK | Anak | Tanggal lahir | RT |
|---|---|---|---|
| NIK ke-1 | anak A | 2023 | 7 |
| | anak B | 2025 | 4 |
| NIK ke-2 | anak C | April 2023 | 6 |
| | anak D | April 2023 | 6 |
...
Ini sejalan dengan [`rujukan/migrasi-data.md`](../rujukan/migrasi-data.md) bagian 6:
...
Saat T1, `lint:check` memberi 25 error dan `types:check` 50 error, seluruhnya `Cannot find module` pada `resources/js/routes/` dan `resources/js/actions/`. Berkas itu dihasilkan plugin Wayfinder yang memanggil `php artisan` saat Vite berjalan, dan PHP di mesin ini 8.2 sementara `composer.json` menuntut ^8.3 — jadi berkas-berkas itu tidak pernah terbentuk. Seluruh 75 error ada di berkas bawaan *starter kit*, nol di berkas yang ditulis untuk demo.

**Selesai oleh [ADR-0006](../adr/0006-pindah-ke-express-react-postgres.md), bukan oleh perbaikan.** Wayfinder, PHP, dan seluruh halaman *starter kit* ikut terhapus bersama Laravel. Ketiga gerbang kini lulus bersih di `client/` maupun `server/`, tanpa garis dasar dan tanpa pengecualian.
...
[`rujukan/migrasi-data.md`](../rujukan/migrasi-data.md) bagian 5.1 menduga hanya arsip 2025 yang
...
- **`--sidebar-surface` tidak ditambahkan.** [`rujukan/ui-ux.md`](../rujukan/ui-ux.md)
...
- **Baris alasan memakai bentuk z-score, bukan "berat sama dua bulan berturut".**
  Bagian 6.3 memberi dua contoh, tetapi [`rujukan/ui-ux.md`](../rujukan/ui-ux.md)
...
menggulir mendatar — melanggar aturan mengikat [`rujukan/ui-ux.md`](../rujukan/ui-ux.md)
...
- **`ui/pagination.tsx` tidak dibuat**, meski disebut
  [`rujukan/ui-ux.md`](../rujukan/ui-ux.md) bagian 4.2. Daftar terpanjang 101 baris,
...
[`rujukan/ui-ux.md`](../rujukan/ui-ux.md) bagian 7. Ikut hilang antara lain
...
## Ringkasan perbaikan pasca-T7

Setelah T7 selesai, delapan ronde perbaikan berjalan antara 13 dan 19 September 2026. **Tujuh dari delapan menangani persoalan yang sama:** membuat layar Detail anak muat satu layar tanpa menggulir, pada berbagai tingkat zoom peramban.

Catatan rinci tiap ronde — pengukuran SVG, perhitungan piksel, urutan kelas Tailwind — **dipangkas 22 September 2026**. Isinya sudah tidak punya pembaca: yang diputuskan sudah hidup di kode, dan yang salah sudah tidak ada. Uraian lengkapnya tersimpan di riwayat git.

| # | Yang dilaporkan | Sebab yang ditemukan | Penyelesaiannya |
|---|---|---|---|
| 1 | Label z-score pada kurva naik-turun seperti tangga | Jarak angkat dihitung dari ganjil-genap urutan titik, untuk menghindari tabrakan yang **ternyata tidak pernah ada** — label 92 satuan, jarak antar bulan 114 satuan | Selang-selingnya dibuang; jarak angkat dibuat tetap |
| 2 | Detail anak tidak lagi muat satu layar | Tata letak satu layar digerbangi `2xl` (1536 px); di bawah itu yang menggulir bagian dalam `<main>`, bukan halamannya | Ambang diturunkan, perilaku gulirnya diseragamkan |
| 3 | Keputusan ulang pemilik produk | — | Satu layar tanpa gulir tegak **dengan seluruh informasi tetap tampil**; ongkosnya dipindah ke kurva yang harus muat di kolom |
| 4 | Masih menggulir di zoom 100%, angka kurva saling menimpa | Anggaran tinggi dihitung dari 1280 × 720, padahal tinggi tersedia sebenarnya 645 px | Anggaran dihitung ulang dari tinggi sebenarnya |
| 5 | Label Identitas menimpa nilai kolom sebelahnya | Ronde sebelumnya hanya memeriksa tabrakan **di dalam SVG**, tidak di seluruh halaman | Pemeriksaan diperluas ke seluruh halaman |
| 6 | Blok Identitas terbaca seperti borang isian | Delapan garis bawah selebar kolom di atas data baca-saja — afordansi yang keliru | Garis bawah dibuang; satu dari empat saran dibatalkan setelah membaca kode |
| 7 | Susunan benar di zoom 90%, salah di zoom 100% | Ambang 1280 px kena lingkaran setan bilah gulir, ditambah urutan kelas Tailwind yang keliru | Ambang dan urutan kelas diperbaiki |
| 8 | Keluhan sama, sebab berbeda | Ambang 1240 px diturunkan dari asumsi yang salah; jendela pengguna jatuh **di antara** dua ambang | Ambang 1088 px, dan batas tinggi kurva yang tidak disadari ikut dilepas |

**Pelajaran yang layak dibawa** — satu-satunya bagian dari delapan ronde ini yang masih berguna:

- **Ukur di peramban, jangan hitung dari angka bulat.** Tiga ronde terbuang karena anggaran tata letak dihitung dari 1280 × 720 dan 1536 px, padahal tinggi jendela sebenarnya 645 px dan bilah gulir menggeser ambangnya.
- **Zoom peramban menggeser ambang CSS.** Pengguna yang memakai zoom 90% dan 100% melihat tata letak berbeda. Uji di keduanya, bukan di salah satunya.
- **Periksa tabrakan di seluruh halaman, bukan di dalam satu komponen.** Ronde 5 ada karena ronde 4 hanya memeriksa di dalam SVG.
